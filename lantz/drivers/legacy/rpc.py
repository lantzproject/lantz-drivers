...
class auth_flavor(enum.IntEnum):
    AUTH_NULL = 0
    AUTH_UNIX = 1
    AUTH_SHORT = 2
    AUTH_DES = 3
...
class Packer(xdrlib.Packer):
    ...
    def pack_replyheader(self, xid, verf):
        self.pack_uint(xid)
        self.pack_enum(msg_type.REPLY)
        self.pack_uint(reply_stat.MSG_ACCEPTED)
        self.pack_auth(verf)
        self.pack_enum(accept_stat.SUCCESS)
...
class Client:
    ...
    def mkcred(self):
        ...
        if self.cred is None:
            self.cred = (auth_flavor.AUTH_NULL, make_auth_null())
        return self.cred

    def mkverf(self):
        # Override this to use a more powerful verifier
        if self.verf is None:
            self.verf = (auth_flavor.AUTH_NULL, make_auth_null())
        return self.verf
...
class PartialPortMapperClient:
    ...
    def set(self, mapping):
        return self.make_call(PMAP_VERS.PMAPPROC_SET, mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def unset(self, mapping):
        return self.make_call(PMAP_VERS.PMAPPROC_UNSET, mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def get_port(self, mapping):
        return self.make_call(PMAP_VERS.PMAPPROC_GETPORT, mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def dump(self):
        return self.make_call(PMAP_VERS.PMAPPROC_DUMP, None,
                              None,
                              self.unpacker.unpack_pmaplist)

    def callit(self, ca):
        return self.make_call(PMAP_VERS.PMAPPROC_CALLIT, ca,
...
class Server:
    ...
    def handle(self, call):
        ...
        if temp != RPCVERSION:
            self.packer.pack_uint(reply_stat.MSG_DENIED)
            self.packer.pack_uint(reject_stat.RPC_MISMATCH)
            self.packer.pack_uint(RPCVERSION)
            self.packer.pack_uint(RPCVERSION)
            return self.packer.get_buf()
        self.packer.pack_uint(reply_stat.MSG_ACCEPTED)
        self.packer.pack_auth((auth_flavor.AUTH_NULL, make_auth_null()))
        prog = self.unpacker.unpack_uint()
        if prog != self.prog:
            self.packer.pack_uint(accept_stat.PROG_UNAVAIL)
            return self.packer.get_buf()
        vers = self.unpacker.unpack_uint()
        if vers != self.vers:
            self.packer.pack_uint(accept_stat.PROG_MISMATCH)
        ...
        try:
            ...
            self.packer.pack_uint(accept_stat.PROC_UNAVAIL)
            return self.packer.get_buf()
        ...
        try:
            ...
            self.packer.pack_uint(reply_stat.MSG_ACCEPTED)
            self.packer.pack_auth((auth_flavor.AUTH_NULL, make_auth_null()))
            self.packer.pack_uint(accept_stat.GARBAGE_ARGS)
        return self.packer.get_buf()
