...
def errcheck(ret, func, args):
    if ret == -1:
        try:
            ...
            raise U12Exception(
                "Exodriver returned an error, but LabJackPython is unable to read the error code. Upgrade to Python 2.6 for this functionality.")
    else:
        return ret
...
if os.name == 'posix':
    try:
        ...
        raise U12Exception(
            "Could not load the Linux SO for some reason other than it not being installed. Ethernet connectivity only.\n\n    The error was: %s" % e)
    ...
    try:
        ...
        raise U12Exception(
            "Could not load the Mac Dylib for some reason other than it not being installed. Ethernet connectivity only.\n\n    The error was: %s" % e)
else:
    try:
...
class U12(object):
    ...
    def rawReadSerial(self):
        ...
        return struct.unpack(">I", struct.pack("BBBB", results['DataByte3'], results['DataByte2'], results['DataByte1'],
                                               results['DataByte0']))[0]
    ...
    def rawAISample(self, channel0PGAMUX=8, channel1PGAMUX=9, channel2PGAMUX=10, channel3PGAMUX=11, UpdateIO=False,
                    LEDState=True, IO3toIO0States=0, EchoValue=0):
        """
    ...
    def rawDIO(self, D15toD8Directions=0, D7toD0Directions=0, D15toD8States=0, D7toD0States=0,
    ...
    def rawCounterPWMDIO(self, D15toD8Directions=0, D7toD0Directions=0, D15toD8States=0, D7toD0States=0,
                         IO3toIO0DirectionsAndStates=0, ResetCounter=False, UpdateDigital=0, PWMA=0, PWMB=0):
        """
    ...
    def rawAIBurst(self, channel0PGAMUX=8, channel1PGAMUX=9, channel2PGAMUX=10, channel3PGAMUX=11, NumberOfScans=8,
                   TriggerIONum=0, TriggerState=0, UpdateIO=False, LEDState=True, IO3ToIO0States=0,
                   FeatureReports=False, TriggerOn=False, SampleInterval=15000):
        """
    ...
    def rawAIContinuous(self, channel0PGAMUX=8, channel1PGAMUX=9, channel2PGAMUX=10, channel3PGAMUX=11,
                        FeatureReports=False, CounterRead=False, UpdateIO=False, LEDState=True, IO3ToIO0States=0,
                        SampleInterval=15000):
        """
    ...
    def rawWatchdog(self, IgnoreCommands=False, D0Active=False, D0State=False, D1Active=False, D1State=False,
                    D8Active=False, D8State=False, ResetOnTimeout=False, WatchdogActive=False, Timeout=60):
        """
    ...
    def rawAsynch(self, Data, AddDelay=False, TimeoutActive=False, SetTransmitEnable=False, PortB=False,
        ...
        bfLabels = ["Timeout Error Flag", "STRT Error Flag", "FRM Error Flag", "RXTris Error Flag", "TETris Error Flag",
                    "TXTris Error Flag"]
    ...
    def rawSPI(self, Data, AddMsDelay=False, AddHundredUsDelay=False, SPIMode='A', NumberOfBytesToWriteRead=0,
               ControlCS=False, StateOfActiveCS=False, CSLineNumber=0):
        """
    ...
    def rawSHT1X(self, Data=[3, 0, 0, 0], WaitForMeasurementReady=True, IssueSerialReset=False, Add1MsDelay=False,
                 Add300UsDelay=False, IO3State=1, IO2State=1, IO3Direction=1, IO2Direction=1, NumberOfBytesToWrite=1,
    ...
    def eAnalogOut(self, analogOut0, analogOut1, idNum=None, demo=0):
        ...
        if ON_WINDOWS:
            ljid = ctypes.c_long(idNum)
            ecode = staticLib.EAnalogOut(ctypes.byref(ljid), demo, ctypes.c_float(analogOut0),
    ...
    def aiSample(self, numChannels, channels, idNum=None, demo=0, stateIOin=0, updateIO=0, ledOn=0, gains=[0, 0, 0, 0],
                 disableCal=0):
        """
        ...
        ecode = staticLib.AISample(ctypes.byref(idNum), demo, ctypes.byref(stateIOin), updateIO, ledOn, numChannels,
                                   ctypes.byref(channelsArray), ctypes.byref(gainsArray), disableCal,
                                   ctypes.byref(overVoltage), ctypes.byref(voltages))

        if ecode != 0: raise U12Exception(ecode)

        return {"idnum": idNum.value, "stateIO": stateIOin.value, "overVoltage": overVoltage.value,
                "voltages": voltages[0:numChannels]}

    def aiBurst(self, numChannels, channels, scanRate, numScans, idNum=None, demo=0, stateIOin=0, updateIO=0, ledOn=0,
                gains=[0, 0, 0, 0], disableCal=0, triggerIO=0, triggerState=0, timeout=1, transferMode=0):
        """
        ...
        ecode = staticLib.AIBurst(ctypes.byref(idNum), demo, stateIOin, updateIO, ledOn, numChannels,
                                  ctypes.byref(channelsArray), ctypes.byref(gainsArray), ctypes.byref(scanRate),
                                  disableCal, triggerIO, triggerState, numScans, timeout, ctypes.byref(voltages),
                                  ctypes.byref(stateIOout), ctypes.byref(overVoltage), transferMode)

        if ecode != 0: raise U12Exception(ecode)

        return {"idnum": idNum.value, "scanRate": scanRate.value, "voltages": voltages, "stateIOout": stateIOout,
                "overVoltage": overVoltage.value}

    def aiStreamStart(self, numChannels, channels, scanRate, idNum=None, demo=0, stateIOin=0, updateIO=0, ledOn=0,
                      gains=[0, 0, 0, 0], disableCal=0, readCount=0):
        """
        ...
        ecode = staticLib.AIStreamStart(ctypes.byref(idNum), demo, stateIOin, updateIO, ledOn, numChannels,
                                        ctypes.byref(channelsArray), ctypes.byref(gainsArray), ctypes.byref(scanRate),
                                        disableCal, 0, readCount)
    ...
    def aiStreamRead(self, numScans, localID=None, timeout=1):
        ...
        ecode = staticLib.AIStreamRead(localID, numScans, timeout, ctypes.byref(voltages), ctypes.byref(stateIOout),
                                       ctypes.byref(reserved), ctypes.byref(ljScanBacklog), ctypes.byref(overVoltage))

        if ecode != 0: raise U12Exception(ecode)  # TODO: Switch this out for exception

        return {"voltages": voltages, "stateIOout": stateIOout, "reserved": reserved.value,
                "ljScanBacklog": ljScanBacklog.value, "overVoltage": overVoltage.value}
    ...
    def aoUpdate(self, idNum=None, demo=0, trisD=None, trisIO=None, stateD=None, stateIO=None, updateDigital=0,
                 resetCounter=0, analogOut0=0, analogOut1=0):
        """
        Name: U12.aoUpdate()
        ...
        # Create arrays and other ctypes
        ecode = staticLib.AOUpdate(ctypes.byref(idNum), demo, trisD, trisIO, ctypes.byref(stateD),
                                   ctypes.byref(stateIO), updateDigital, resetCounter, ctypes.byref(count),
    ...
    def asynchConfig(self, fullA, fullB, fullC, halfA, halfB, halfC, idNum=None, demo=None, timeoutMult=1, configA=0,
                     configB=0, configTE=0):
        """
        ...
        ecode = staticLib.AsynchConfig(ctypes.byref(idNum), demo, timeoutMult, configA, configB, configTE, fullA, fullB,
                                       fullC, halfA, halfB, halfC)

        if ecode != 0: raise U12Exception(ecode)  # TODO: Switch this out for exception

        return {"idNum": idNum.value}

    def asynch(self, baudrate, data, idNum=None, demo=0, portB=0, enableTE=0, enableTO=0, enableDel=0, numWrite=0,
               numRead=0):
        """
        Name: U12.asynchConfig(fullA, fullB, fullC, halfA, halfB, halfC, idNum=None, demo=None, timeoutMult=1, configA=0, configB=0, configTE=0)
        ...
        ecode = staticLib.Asynch(ctypes.byref(idNum), demo, portB, enableTE, enableTO, enableDel, baudrate, numWrite,
                                 numRead, ctypes.byref(dataArray))
    ...
    def counter(self, idNum=None, demo=0, resetCounter=0, enableSTB=1):
        ...
        print(idNum)
        ecode = staticLib.Counter(ctypes.byref(idNum), demo, ctypes.byref(stateD), ctypes.byref(stateIO), resetCounter,
                                  enableSTB, ctypes.byref(count))
    ...
    def digitalIO(self, idNum=None, demo=0, trisD=None, trisIO=None, stateD=None, stateIO=None, updateDigital=0):
        ...
        ecode = staticLib.DigitalIO(ctypes.byref(idNum), demo, ctypes.byref(trisD), trisIO, ctypes.byref(stateD),
                                    ctypes.byref(stateIO), updateDigital, ctypes.byref(outputD))
        if ecode != 0: raise U12Exception(ecode)

        return {"idnum": idNum.value, "trisD": trisD.value, "stateD": stateD.value, "stateIO": stateIO.value,
    ...
    def getWinVersion(self):
        ...
        ecode = staticLib.GetWinVersion(ctypes.byref(majorVersion), ctypes.byref(minorVersion),
                                        ctypes.byref(buildNumber), ctypes.byref(platformID),
                                        ctypes.byref(servicePackMajor), ctypes.byref(servicePackMinor))

        if ecode != 0: raise U12Exception(ecode)

        return {"majorVersion": majorVersion.value, "minorVersion": minorVersion.value,
                "buildNumber": buildNumber.value, "platformID": platformID.value,
                "servicePackMajor": servicePackMajor.value, "servicePackMinor": servicePackMinor.value}
    ...
    def listAll(self):
        ...
        ecode = staticLib.ListAll(ctypes.byref(productIDList), ctypes.byref(serialnumList), ctypes.byref(localIDList),
                                  ctypes.byref(powerList), ctypes.byref(calMatrix), ctypes.byref(numberFound),
                                  ctypes.byref(reserved), ctypes.byref(reserved))
        if ecode != 0: raise U12Exception(ecode)
    ...
    def pulseOut(self, bitSelect, numPulses, timeB1, timeC1, timeB2, timeC2, idNum=None, demo=0, lowFirst=0):
        ...
        ecode = staticLib.PulseOut(ctypes.byref(idNum), demo, lowFirst, bitSelect, numPulses, timeB1, timeC1, timeB2,
                                   timeC2)
        if ecode != 0: raise U12Exception(ecode)
    ...
    def pulseOutStart(self, bitSelect, numPulses, timeB1, timeC1, timeB2, timeC2, idNum=None, demo=0, lowFirst=0):
        ...
        ecode = staticLib.PulseOutStart(ctypes.byref(idNum), demo, lowFirst, bitSelect, numPulses, timeB1, timeC1,
                                        timeB2, timeC2)
        if ecode != 0: raise U12Exception(ecode)
    ...
    def sht1X(self, idNum=None, demo=0, softComm=0, mode=0, statusReg=0):
        ...
        ecode = staticLib.SHT1X(ctypes.byref(idNum), demo, softComm, mode, statusReg, ctypes.byref(tempC),
                                ctypes.byref(tempF), ctypes.byref(rh))
        if ecode != 0: raise U12Exception(ecode)
    ...
    def shtComm(self, numWrite, numRead, datatx, idNum=None, softComm=0, waitMeas=0, serialReset=0, dataRate=0):
        ...
        ecode = staticLib.SHTComm(ctypes.byref(idNum), softComm, waitMeas, serialReset, dataRate, numWrite, numRead,
                                  ctypes.byref(datatx), ctypes.byref(datarx))
        if ecode != 0: raise U12Exception(ecode)
    ...
    def synch(self, mode, numWriteRead, data, idNum=None, demo=0, msDelay=0, husDelay=0, controlCS=0, csLine=None,
              csState=0, configD=0):
        """
        ...
        ecode = staticLib.Synch(ctypes.byref(idNum), demo, mode, msDelay, husDelay, controlCS, csLine, csState, configD,
                                numWriteRead, ctypes.byref(cData))
        if ecode != 0: raise U12Exception(ecode)
    ...
    def watchdog(self, active, timeout, activeDn, stateDn, idNum=None, demo=0, reset=0):
        ...
        ecode = staticLib.Watchdog(ctypes.byref(idNum), demo, active, timeout, reset, activeDn[0], activeDn[1],
                                   activeDn[2], stateDn[0], stateDn[1], stateDn[2])
        if ecode != 0: raise U12Exception(ecode)
    ...
    def readMem(self, address, idnum=None):
        ...
        ec = staticLib.ReadMem(ctypes.byref(ljid), ctypes.c_long(address), ctypes.byref(ad3), ctypes.byref(ad2),
                               ctypes.byref(ad1), ctypes.byref(ad0))
    ...
    def writeMem(self, address, data, idnum=None, unlocked=False):
        ...
        ljid = ctypes.c_ulong(idnum)
        ec = staticLib.WriteMem(ctypes.byref(ljid), int(unlocked), address, data[3] & 0xff, data[2] & 0xff,
