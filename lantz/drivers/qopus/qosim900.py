...
class QOSIM900(MessageBasedDriver):
    ...
    AMP_CHS = {0: 2, 1: 3, 2: 4, 3: 5, 4: 6, 5: 7, 6: 8}
    SIM922_ch = 1
    ...
    def reset_bias(self, key):
        if not key in list(self.AMP_CHS.keys()): raise Exception(
            'Specified <key> not in channel list.  Please check AMP_CH')
    ...
    def adc_voltage(self, key):
        if not self._buffered_adc_gain in ['low', 'high']:
            raise Exception('adc_gain unknown, please set using <inst>.set_adc_gain(channel, state)')
        else:
